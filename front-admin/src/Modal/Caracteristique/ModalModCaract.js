...
function ModalModCaract({Index}) {
    const [show,setShow] = useState(false);
    const handleClose = () => setShow(false)
    const handleShow = () => setShow(true)
    const [surface, setsurface] = useState(Index.surface);
    const [nombreDePoule, setnombreDePoule] = useState(Index.nombreDePoule);
    const [humidite, sethumidite] = useState(Index.humidite);
    const [lumiere, setlumiere] = useState(Index.lumiere);
    const [intensite, setintensite] = useState(Index.intensite);
    const [temperature, settemperature] = useState(Index.temperature);
    //controlleur input
 ...
 const handleSubmit = async (e) => {
   ...
   try {
     const reponse = await axios.put(
       `http://localhost:4000/api/caractere/Modifier/${Index.id}`,
       {
        surface,
        nombreDePoule,
        humidite,
