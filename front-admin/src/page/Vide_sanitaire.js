...
const handleConfirmDelete = async () => {
  try {
    const response = await axios.delete(`http://backend:4000/api/vide_sanitaire/supprimer/${deleteVideID}`);
  ...
  useEffect(()=>{
    const fecthAllVide = async ()=>{
        try {
            const res = await fetch("http://backend:4000/api/vide_sanitaire/");
